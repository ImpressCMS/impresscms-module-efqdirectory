		...
		if ($get_catid == 0) {
			...
			while(list($itemid, $logourl, $uid, $status, $created, $ltitle, $hits, $rating, $votes, $type, $dirid, $description) = $xoopsDB->fetchRow($result)) {
				...
				if ($xoopsModuleConfig['showdatafieldsincat'] == '1') {
					...
					$sql .= "LEFT JOIN ".$xoopsDB->prefix("efqdiralpha1_data")." d ON (t.dtypeid=d.dtypeid AND d.itemid=".$itemid.") ";
			...
			if($numrows>0){
				...
				$sql = "SELECT i.itemid, i.logourl, i.uid, i.status, i.created, i.title, i.hits, i.rating, i.votes, i.typeid, i.dirid, t.level, txt.description, x.cid FROM ".$xoopsDB->prefix("efqdiralpha1_item_x_cat")." x, ".$xoopsDB->prefix("efqdiralpha1_items")." i LEFT JOIN ".$xoopsDB->prefix("efqdiralpha1_itemtypes")." t ON (t.typeid=i.typeid) LEFT JOIN ".$xoopsDB->prefix("efqdiralpha1_item_text")." txt ON (txt.itemid=i.itemid) WHERE i.itemid=x.itemid AND x.cid=$get_catid AND x.active='1' AND i.status='2' ORDER BY $orderby";
