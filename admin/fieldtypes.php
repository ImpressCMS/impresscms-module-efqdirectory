...
function fieldtypesConfig()
{
    ...
    if ( $numrows > 0 ) {
		while(list($typeid, $title, $type, $descr, $ext, $status) = $xoopsDB->fetchRow($result)) {
			...
			echo "<tr><td class=\"even\" valign=\"top\"><a href=\"".XOOPS_URL."/modules/".$moddir."/admin/fieldtypes.php?op=view&typeid=$typeid\">$title</a></td><td class=\"even\" valign=\"top\">$type</td><td class=\"even\" valign=\"top\">$ext</td><td class=\"even\" valign=\"top\">$statusyn</td>";
			echo "</td></tr>\n";
		}
	} else {
...
function viewFieldtype()
{
	...
	echo "<table width='100%' border='0' cellspacing='1' class='outer'><tr><td>";
	$result = $xoopsDB->query("SELECT typeid, title, fieldtype, descr, ext, activeyn FROM ".$xoopsDB->prefix("efqdiralpha1_fieldtypes")." WHERE typeid='".$get_typeid."'") or $eh->show("0013");
	$numrows = $xoopsDB->getRowsNum($result);
    if ( $numrows > 0 ) {
...
function addFieldtype()
{
	...
	$sql = sprintf("INSERT INTO %s (typeid, title, fieldtype, descr, ext, activeyn) VALUES (%u, '%s', '%s', '%s', '%s', '%s')", $xoopsDB->prefix("efqdiralpha1_fieldtypes"), $newid, $p_title, $p_fieldtype, $p_descr, $p_ext, $p_status);
